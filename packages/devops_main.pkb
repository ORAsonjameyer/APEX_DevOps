...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "WKSP_DEVOPS"."DEVOPS_MAIN" AS
  ...
  END query_objects;
    ...
    PROCEDURE export_app(
      ...
      l_app_id       NUMBER := 110;  -- Replace with your App ID
      l_name         VARCHAR2(255 CHAR);
      l_app_clob     CLOB;
      l_app_blob     BLOB;
      l_commit_details VARCHAR2(32767);  -- Use VARCHAR2 to construct the JSON
    BEGIN
      ...
      -- Create the commit details JSON object as a VARCHAR2
      l_commit_details := '{"message": "DBMS_CLOUD_REPO commit", "author": "ORAsonjameyer", "email": "sonja.meyer@oracle.com"}';
...
PROCEDURE exp_app1
    IS
        ...
        repoName       varchar2(50 CHAR) := 'APEX_DEVOPS';    -- Name of the GitHub Repository
        repoOwner      varchar2(50 CHAR) := 'ORAsonjameyer';  -- Name of the GitHub Repository Owner

        l_file         apex_t_export_files;
        l_app_id       number := 110;                       -- App-ID of the exported application
